 ...
 * http://afraware.com/product/jquery-plugin/dialogbox
 * https://github.com/afraware/dialogbox
 */
...
(function ($) {
    ...
    $.fn.dialogbox = function ($options, $callback) {
        ...
        switch($settings['type']) {
            ...
            // Prompts
             case 'text/add':
                ...
                $($id + ' .dialogbox-button-bar').html('').append('<div class="dialogbox-btn2">OK</div>');
                $($id + ' .dialogbox-prompt').html('').show().append('<input id="inputTextValue2" type="text" placeholder="' +
                $placeholder + '">');
                break;
            case 'text':
                var $placeholder = ($settings['placeholder'] == null) ? 'Type...' : $settings['placeholder'];
                $($id + ' .dialogbox-button-bar').html('').append('<div class="dialogbox-btn">OK</div>');
                $($id + ' .dialogbox-prompt').html('').show().append('<input id="inputTextValue" type="text" placeholder="' +
                $placeholder + '">');
                break;
            case 'text/edit':
                var $placeholder = ($settings['placeholder'] == null) ? 'Type...' : $settings['placeholder'];
                $($id + ' .dialogbox-button-bar').html('').append('<div class="dialogbox-btn">OK</div>');
                $($id + ' .dialogbox-prompt').html('').show().append('<input id="EditTextBox" type="text" placeholder="' +
                $placeholder + '">');
                var EditValue = $settings['initialEditValue'];
                $("#EditTextBox").val(EditValue);
                break;
            case 'select':
        ...
        if($callback && $callback.constructor && $callback.call && $callback.apply) {
            ...
            // Buttons
            if($settings['type'] == 'text' || $settings['type'] == 'text/edit' || $settings['type'] == 'yes/no' ) {
            ...
            }  else if($settings['type'] == 'text/add') {
                $(document).on('click', $id + ' .dialogbox-btn2', function () {
                    var $btn = 'ok';
                    var $return = $($id + ' input').val();
                    if($return == '') {
                        $btn = "close";
